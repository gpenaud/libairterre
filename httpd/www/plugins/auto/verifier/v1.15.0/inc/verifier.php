 ...
 *
 * @note
 * Cette fonction nécessite le plugin Yaml
 * ET un des plugins fournissant _T_ou_typo() : Bonux ou Saisies.
 *
...
function verifier_charger_infos($type_verif, $repertoire = 'verifier') {
	$verif = array();
	include_spip('inc/utils'); // pour test_plugin_actif()

	if (test_plugin_actif('yaml')) {
		include_spip('inc/yaml');
		include_spip('inc/filtres'); // pour appliquer_filtre()
		$fichier = find_in_path("$repertoire/$type_verif.yaml");

		$verif = yaml_decode_file($fichier);
		if (is_array($verif)) {
			$verif['titre']       = (isset($verif['titre'])       and $verif['titre'])       ? appliquer_filtre($verif['titre'], '_T_ou_typo', true) : $type_verif;
			$verif['description'] = (isset($verif['description']) and $verif['description']) ? appliquer_filtre($verif['description'], '_T_ou_typo', true) : '';
			$verif['icone']       = (isset($verif['icone'])       and $verif['icone'])       ? appliquer_filtre($verif['icone'], '_T_ou_typo', true) : '';
		}
	}
	return $verif;
}
